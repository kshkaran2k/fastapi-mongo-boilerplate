...
from app.core.constants import ORDERS_COLLECTION, ORDERS_PREFIX, ORDERS_TAG
...
router = APIRouter(prefix=ORDERS_PREFIX, tags=[ORDERS_TAG])
...
async def create_order(payload: OrderCreate):
    ...
    ok, _ = await MongoDB.insert_one(ORDERS_COLLECTION, order)
    if not ok:
...
async def get_order(order_id: str):
    ok, order = await MongoDB.find_one(ORDERS_COLLECTION, {"order_id": order_id})
    if not ok:
